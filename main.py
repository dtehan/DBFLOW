...
class ExpectationFlow(Flow[ExpectationState]):
    ...
    @listen(or_(get_table_description_bedrock, get_table_description_openai))
    def build_table_report(self):
        ...
        if get_crew() == "expectation":
            ...
            self.state.all_tables_expectation_str = self.state.all_tables_expectation_str.replace(
                "```python", "\n")
            self.state.all_tables_expectation_str = self.state.all_tables_expectation_str.replace(
                "```", "\n")
            script = script + self.state.all_tables_expectation_str + "\n"
            # add the closing part of the script
            script = script + \
                open("crews/expectation_crew/config/post_script.txt", "r").read() + "\n"

            # write the script to a file
            print("#################################")
            print("writting the great expectation script")
            f = open(
                f"crews/expectation_crew/Output/{self.state.schema_name}_script.py", "w")
            f.write(script)
            f.close()
